...
#Loading the data file with attributes name
...
# Splitting a dataset for k = 5 folds cross validation
...
# Implementing an algorithm using a single split
def evaluate_algorithm_single(dataset, algorithm, *args):
    train_set, test_set = train_test_split(dataset, 0.80) # Data splitting
    scores = list()
...
# Implementing an algorithm using cross validation split
...
# Splitting the dataset based on an attribute and an attribute value
...
# Implementing the Entropy for a split dataset  and decide the root for the decision tree
def entropy(groups, classes, b_score):
    ...
    for group in groups:
        ...
        # weight the group score by its relative size that is Entropy gain
...
def get_split(dataset, split_parameter):
    if split_parameter == 'entropy':  #parameter entropy
    ...
    elif split_parameter == 'gini':  # parameter gini
        class_values = list(set(row[-1] for row in dataset))
...
def split(node, max_depth, min_size, depth):
    ...
    del (node['groups'])
    # check  whether there is no split
...
# Making prediction with a decision tree
...
# Classification and Regression Tree Algorithm for handling discrete and continous data
...
# Datasets-iris.data and house-votes-84.data
# loading and preparing data
fetch_input = sys.argv[1] #Input for data file 
filename = fetch_input #The data filename on which decision tree needs to be built
dataset = load_csv(filename)
# Tree model creation on training set
n_folds = 5
max_depth = 3
min_size = 1
split_parameter = 'entropy'  # 'entropy'/'gini'
...
# Calculate scores for k cross validation by setting n_folds value
print('  ')
print('Printing k-cross validation result')
